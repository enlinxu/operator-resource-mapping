package controllers

import (
	"context"
	"fmt"
	turbonomiciov1alpha1 "github.com/enlinxu/operator-resource-mapping/api/v1alpha1"
	v1 "github.com/jaegertracing/jaeger-operator/apis/v1"
	"k8s.io/apimachinery/pkg/api/errors"
	metav1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	"k8s.io/apimachinery/pkg/runtime"
	ctrl "sigs.k8s.io/controller-runtime"
	"sigs.k8s.io/controller-runtime/pkg/client"
	"sigs.k8s.io/controller-runtime/pkg/log"
	"sigs.k8s.io/controller-runtime/pkg/reconcile"
)

type JaegerReconciler struct {
	client.Client
	Scheme *runtime.Scheme
}

//+kubebuilder:rbac:groups=turbonomic.io,resources=operatorresourcemappings,verbs=get;list;watch;create;update;patch;delete
//+kubebuilder:rbac:groups=turbonomic.io,resources=operatorresourcemappings/status,verbs=get;update;patch
//+kubebuilder:rbac:groups=turbonomic.io,resources=operatorresourcemappings/finalizers,verbs=update

// Reconcile is part of the main kubernetes reconciliation loop which aims to
// move the current state of the cluster closer to the desired state.
// TODO(user): Modify the Reconcile function to compare the state specified by
// the OperatorResourceMapping object against the actual cluster state, and then
// perform operations to make the cluster state reflect the state specified by
// the user.                                                     kube
//
// For more details, check Reconcile and its Result here:
// - https://pkg.go.dev/sigs.k8s.io/controller-runtime@v0.11.0/pkg/reconcile
func (r *JaegerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	_ = log.FromContext(ctx)
	fmt.Printf("In Jaeger reconcile")
	cr := &v1.Jaeger{}
	err := r.Get(context.TODO(), req.NamespacedName, cr)
	if err != nil {
		if errors.IsNotFound(err) {
			return reconcile.Result{}, nil
		}
		// Other errors reading the object - requeue the request.
		return reconcile.Result{}, err
	}
	orm := turbonomiciov1alpha1.OperatorResourceMapping{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "jaegers.jaegertracing.io",
			Namespace: req.NamespacedName.Namespace,
		},
		Spec: turbonomiciov1alpha1.OperatorResourceMappingSpec{
			ResourceMappings: []turbonomiciov1alpha1.ResourceMapping{
				{
					SrcResourceSpec: turbonomiciov1alpha1.SourceResourceSpec{
						Kind:           "Deployment",
						ComponentNames: []string{"jaeger"},
					},
					ResourceMappingTemplates: []turbonomiciov1alpha1.ResourceMappingTemplate{
						{
							SrcPath:  ".spec.template.spec.containers[?(@.name==\"{{.componentName}}\")].resources",
							DestPath: ".spec.resources",
						},
					},
				},
			},
		},
	}

	err = r.Client.Create(ctx, &orm)
	if err != nil {
		fmt.Printf("Failed to create the ORM instance for Jaeger %s", err)

	}
	fmt.Printf("Created Jaeger ORM instance")

	// TODO(user): your logic here

	return ctrl.Result{}, nil
}

// SetupWithManager sets up the controller with the Manager.
func (r *JaegerReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&v1.Jaeger{}).
		Complete(r)
}
